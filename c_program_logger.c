...
void get_system_property(const char *property, char *result, size_t size) {
    ...
    if (fp != NULL) {
        if (fgets(result, size, fp) != NULL) {
            ...
            log_error("Failed to read system property.");
        }
        pclose(fp);
    } 
	else {
        log_error("Failed to execute getprop command.");
    }
}
...
void uninstall_package(const char *package_name) {
    ...
    if (result != 0) {
        ...
        print_centered(error_message);
        log_error(error_message); // Log error
    }
    else {
...
void get_device_serial(char *serial, size_t size) {
    ...
    if (fp == NULL) {
        ...
        log_error("Failed to get device serial.");
        return;
    }
    if (fgets(serial, size, fp) != NULL) {
        ...
        log_error("Failed to read device serial.");
    }
    pclose(fp);
}
...
int load_debloat_list(const char *filename, char debloat_list[][BUFFER_SIZE], int max_packages) {
    ...
    if (!file) {
        ...
        log_error("Debloat list file could not be opened.");
        return 0;
    }
...
int main(int argc, char *argv[]) {
    ...
    if (argc >= 2) {
        ...
        if (package_count == 0) {
            ...
            log_error("No packages loaded, falling back to default list.");
        }
    }
